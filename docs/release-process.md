...
2. Move completed release notes under a new heading:
   - `## <MAJOR.MINOR.PATCH>` (example: `## 2.5.1`)
...
```bash
gh release create v<MAJOR.MINOR.PATCH> --title "v<MAJOR.MINOR.PATCH>" --generate-notes
gh release edit v<MAJOR.MINOR.PATCH> --notes "<paste changelog section text here>"
```

Required: release body should contain the actual `## <MAJOR.MINOR.PATCH>` notes (not only an auto-generated compare link).

## 7) Verify Published Release

```bash
gh release view v<MAJOR.MINOR.PATCH> --json tagName,name,publishedAt,url,body
